...
- [ ] T001 [P] Añadir a `src/types/api.ts` los tipos del contrato (ver `contracts/api.md`):
...
- [ ] T002 Editar `prisma/schema.prisma`: añadir a `RealCost` los campos `description String`
  e `incurredOn DateTime @db.Date` (fecha del coste). Regenerar el client (`prisma generate`).
- [ ] T003 Crear `prisma/migrations/<ts>_real_cost_costs_and_immutability/migration.sql`
...
- [ ] T004 [P] Escribir `tests/budget-real-costs.test.ts` (rojo): `accumulatedCostCents`
...
- [ ] T005 Implementar `src/lib/budget/real-costs.ts` (verde): `accumulatedCostCents(costs)`,
...
- [ ] T006 [US1] Crear `src/server/projects/real-costs.ts` con `getBudgetLineDetail(prisma,
...
- [ ] T007 [US1] En `src/server/projects/real-costs.ts`, `addRealCost(...)`: valida
...
- [ ] T008 [US1] En `src/server/routes/projects.ts`: `GET /projects/:projectId/budget/lines/
...
- [ ] T009 [P] [US1] Hooks `src/hooks/useBudgetLineDetail.ts` (query `['budget-line', lineId]`)
  y `src/hooks/useAddRealCost.ts` (invalida el detalle y `['project-budget', projectId]`).
- [ ] T010 [US1] En `src/pages/ProjectBudgetPage.tsx`: abrir **detalle de partida** (modal o
...
- [ ] T011 [US1] `tests/server/project-real-costs.test.ts` (integración US1): imputar 2 costes
...
- [ ] T012 [P] [US1] `tests/frontend/project-budget-detail.test.tsx`: render del detalle con
...
- [ ] T013 [US2] En `src/server/projects/real-costs.ts`, `updateProgress(...)`: valida
  ...
  Devuelve el detalle.
- [ ] T014 [US2] En `src/server/routes/projects.ts`: `PATCH .../lines/:lineId/progress`
  (`'progress.update'`) con Zod (`progressPercent` int 0–100).
- [ ] T015 [P] [US2] Hook `src/hooks/useUpdateProgress.ts` (invalida el detalle de la partida).
- [ ] T016 [US2] En el detalle de partida (`ProjectBudgetPage.tsx`): control "Actualizar
  avance" (0–100) visible para `progress.update`, con autor/fecha del último avance.
- [ ] T017 [US2] Tests en `tests/server/project-real-costs.test.ts`: 40%→60% sustituye con
...
- [ ] T018 [US3] En `src/server/projects/real-costs.ts`, `reverseRealCost(...)`: valida
...
- [ ] T019 [US3] En `src/server/routes/projects.ts`: `POST .../budget/costs/:costId/reversal`
  (`'realCost.reverse'`) con Zod (`reason` no vacío).
- [ ] T020 [P] [US3] Hook `src/hooks/useReverseRealCost.ts` (invalida el detalle de la partida
  y `['project-budget', projectId]`).
- [ ] T021 [US3] En el detalle de partida: botón "Anular" con campo motivo (solo
  `realCost.reverse`); los asientos anulados se muestran tachados.
- [ ] T022 [US3] Tests en `tests/server/project-real-costs.test.ts`: anular crea
...
- [ ] T023 [P] Revisar mensajes de error y textos UI en español; sin `console.log` ni código
  muerto; `pnpm typecheck` + `pnpm lint` + `pnpm test` en verde.
- [ ] T024 Validar en navegador el flujo C según `quickstart.md` (imputar → avance → anular)
  con cuentas demo; capturar evidencia.
- [ ] T025 [P] Anotar la sesión S14 en `docs/diario.md`.
