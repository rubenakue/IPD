...
5. **Middleware `validate(schema)`**: recibe un esquema Zod (para `body`/`query`/`params`), valida y, si falla, crea un `ApiError` `VALIDATION_ERROR`. Es la pieza reutilizable que S09 usa en `/api/auth/login`. En S08 no hay endpoint de entrada en producción (solo `/health`, sin input), así que la pieza se entrega lista y se cubre con el test (montándola en una app de prueba) — no se añade ningún endpoint de relleno.
