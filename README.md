...
- `pnpm dev:server` — arranca la API Express en desarrollo
...
2. **Configura la conexión**: copia `.env.example` a `.env` y ajusta `DATABASE_URL` con tu contraseña. Define también `SESSION_SECRET` con un valor local de al menos 32 caracteres. El `.env` no se versiona.
...
Comprueba login, sesión y logout con una cuenta de demo:
